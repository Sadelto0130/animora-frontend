...
const BlogInteraction = ({post}) => {
  ...
  const { user } = useAuth()
  const {setEditorState, likePostUser, desLikePostUser} = useBlog()
  const [countLike, setCountLike] = useState(parseInt(post?.total_likes) || 0)
  const [liked, setLiked] = useState(false)

  const {post_id, total_likes, comments, title, users, slug, liked_by} = post || {}
    ...
    <Toaster />
      ...
      <div className="flex gap-6 justify-between"> 
        ...
        <div className="flex gap-6 items-center">
          ...
          <Link to={`https://www.facebook.com/sharer/sharer.php?u=${location.href}`} target='_blank'>
