...
const CommentsContainer = () => {
  ...
  useEffect(() => {
    ...
    const handleNewComment = (newComment) => {
      if (newComment?.post_id !== post_id || !newComment.content?.trim()) return;
        ...
        if (newComment.parent_comment_id) {
          const insertRecursively = (comments) => {
            return comments.map(c => {
              ...
              if (c.replies?.length) return { ...c, replies: insertRecursively(c.replies) };
              return c;
            });
          };
  ...
  return (
    <div className={"max-sm:w-full fixed " + (comentarios ? "top-0 sm:right-0" : "top-[100%] sm:right-[-100%]") + " duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
      <div className="relative">
        ...
        <button onClick={()=> setComentarios(preVal => !preVal)} className="absolute top-0 right-0 flex justify-center items-center w-12 h-12 rounded-full bg-grey">
          <i className="fi fi-br-cross text-2xl mt-1"></i>
        </button>
      </div>
      <hr className="border-grey my-8 w-[120%] -ml-10"/>

      <CommentsField action="comentar" idPost={post_id} onNewReply={(newComment) => setAllComments(prev => [newComment, ...prev])}/>
      ...
      {allComments?.length > showComments && (
        <button onClick={() => (showComments + 10 <= allComments.length ? setShowComments(prev => prev + 10) : loadMoreComments())}
